...
[2] 파일 구성 및 역할
| 파일명 | 역할 | 
| run_pipeline.py | 전체 blink 전처리 파이프라인을 실행하는 메인 스크립트, 다중 스레드(ThreadPoolExecutor)를 통해 피험자 데이터를 병렬 처리 | 
| ds003838_pipeline.py | HTTPS 퍼블릭 엔드포인트를 통해 데이터를 스트리밍으로 다운로드하는 모듈 | 
| blink_features.py | Cho (2021) 논문의 방법론(Section 3.2)을 구현하여 blink 신호를 처리하는 핵심 분석 모듈 | 
| labeling.py | K-means(k=2) 알고리즘을 기반으로 피험자별 개인화된 인지 과부하 라벨링을 수행 | 
| plot_blink_spectrogram.py | 추출된 참가자별 blink 스펙트로그램(_spectrogram.npy)을 히트맵 형태로 시각화 | 
| config.py | 전체 파이프라인이 공유하는 환경 변수(S3 버킷 정보, 타임아웃, 대역폭 등) 설정 파일 | 
