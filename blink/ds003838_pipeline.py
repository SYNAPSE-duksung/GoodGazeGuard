...
읽어오는 다운로드 전용 모듈. 설정값은 전부 config.py에서 가져옵니다.

⚠️ 네트워크 행(hang) 방지 배경:
requests.get(..., timeout=(5, 20))을 줘도 "GET 시작"에서 응답도 에러도
없이 영원히 멈추는 경우가 있었습니다. 원인은 두 가지:
  1) requests/urllib3의 timeout은 'TCP 연결 이후'에만 적용되고, 그 이전
     단계인 DNS 조회(getaddrinfo)는 전혀 커버하지 않음.
  2) s3.amazonaws.com이 IPv4/IPv6 주소를 같이 리턴하는데, 네트워크에서
     IPv6 경로가 (거부되지 않고) 조용히 막혀있으면 연결 시도가 응답 없이
     계속 지연될 수 있음.
대응:
  1) IPv4만 쓰도록 강제
  2) 실제 요청을 별도 daemon 스레드에서 실행하고, 스레드 자체에 강제
     마감시간을 걸어서 DNS든 연결이든 응답이든 어디서 멈추든
     HARD_TIMEOUT_SEC 안에 반드시 실패 처리되게 함
  3) 재시도는 여기서 여러 번 하지 않음(MAX_RETRIES=1) — 재시도를 반복하면
     '멈춘 이전 시도'가 daemon 스레드로 계속 쌓여서(강제 종료가 안 됨)
     장시간 실행 시 컴퓨터가 느려지는 원인이 됐음. 실패한 참가자는
     run_pipeline.py의 체크포인트가 다음 실행 때 자동으로 재시도함.
"""
